...
st.set_page_config(page_title="Eva", page_icon="💬", layout="centered")
...
try:
    ...
    st.error(str(erro))
    st.info("Crie um arquivo `.env` na pasta do projeto com `OPENAI_API_KEY=sk-...`")
    st.stop()
...
with st.sidebar:
    ...
    if eva.MODELO not in modelos:
        modelos.insert(0, eva.MODELO)

    modelo = st.selectbox("Modelo", modelos, index=modelos.index(eva.MODELO))
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            for tipo, dado in fluxo:
                if tipo == "ferramenta" and isinstance(dado, dict):
                    usos.append(dado)
                    if mostrar_ferramentas:
                        with area_ferramentas:
                            desenhar_ferramenta(dado)
                elif tipo == "texto" and isinstance(dado, str):
                    texto += dado
