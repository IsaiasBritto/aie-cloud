...
st.set_page_config(page_title="Eva no Azure", page_icon="☁️", layout="centered")

# Liga o Application Insights uma vez por processo. Sem connection string
# configurada, vira no-op — o app roda igual na sua máquina.
if "telemetria_ligada" not in st.session_state:
    st.session_state.telemetria_ligada = telemetria.iniciar("eva-web")
...
with st.sidebar:
    ...
    # Como a aplicação se autenticou de verdade. Em produção isto tem que
    # dizer "identidade" — se disser "FALLBACK", a Managed Identity quebrou e
    # a aplicação está rodando com chave sem ninguém ter percebido.
    modo = conexao["modo_ativo"]
    if "FALLBACK" in modo:
        st.warning(f"🔑 {modo}", icon="⚠️")
    else:
        st.caption(f"🔐 {modo}")

    tel = telemetria.resumo()
    if tel["ligada"]:
        aviso = "" if tel["precos_definidos"] else " (preços não configurados: custo sai zerado)"
        st.caption(f"📊 telemetria ligada{aviso}")
    else:
        st.caption("📊 telemetria desligada")
