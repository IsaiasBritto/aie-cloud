...
diagrama({
  kicker: "Bloco 4 · fecho", dir: "Ponta a ponta",
...
/* ═══════════════ BLOCO 5 · LABORATÓRIO ═══════════════ */
bloco("5", "Bloco 5 · 45 minutos", "Laboratório",
  "Oito módulos, cada um amarrado a um arquivo do repositório\ne a um conceito da aula. Nenhum passo existe só para configurar.");

figura("dc-lab-01-objetivo.png", 2880 / 1436, "Laboratório", "O objetivo",
  "Três degraus, quatro peças construídas, um entregável — e nenhuma permissão nova para o agente");

figura("dc-lab-02-passos.png", 3000 / 2092, "Laboratório", "O passo a passo",
  "docs/01-manual-portal.md · a coluna da direita é o que você diz em voz alta em cada módulo");

figura("dc-lab-03-mapa.png", 2880 / 1498, "Laboratório", "Conceito → código",
  "As duas linhas em destaque são o projeto inteiro: a fronteira e quem a garante");

figura("dc-lab-04-local.png", 2880 / 1528, "Laboratório · antes da nuvem", "Dois minutos na sua máquina",
  "docker compose up · semear · três voltas do agente — e a quarta volta, em que nada acontece");

figura("dc-lab-05-codigo-fronteira.png", 2920 / 1444, "Laboratório · o código", "A fronteira",
  "api/principal.py e gerar_openapi_do_agente.py — instrução vira permissão em oito linhas");

figura("dc-lab-06-codigo-estados.png", 2920 / 1394, "Laboratório · o código", "A máquina de estados",
  "api/modelos.py e api/servicos/fila.py — o que impede o agente de girar em falso a noite inteira");

figura("dc-lab-07-checkpoints.png", 2880 / 1396, "Laboratório", "Checkpoints",
  "Um por módulo · se algum falhar, não siga: o próximo depende dele");

figura("dc-lab-08-erros.png", 2960 / 1428, "Laboratório", "Os erros que vão acontecer",
  "Dois deles não são configuração, são conteúdo — quando aparecerem, pare a aula e mostre");

{
  const s = conteudo("Laboratório", "Ligando as pontas", "O que cada peça do lab prova",
    "Se a turma sair com uma frase por linha desta tabela, o laboratório cumpriu o papel");
  const linhas = [
    ["Módulo 3 · o gatilho", "gatilho/logic-app-eventgrid.json", "Nível 3 — o agente deixa de esperar alguém digitar. O gatilho é um evento, não um horário."],
    ["Módulo 4 · a ferramenta", "agente/openapi-agente.json", "A API tem 14 operações e o agente recebe 5. Ele não vê o que não pode fazer."],
    ["Módulo 6 · a aprovação", "web/aplicacao.py · aba Propostas", "A linha atravessa de memoria-pendente.md para MEMORY.md, assinada por uma pessoa."],
    ["Módulo 6 · a exceção", "api/servicos/fila.py", "Nível 4 — documento parado espera gente. É o que evita o laço caro da madrugada."],
    ["Módulo 7 · a injeção", "api/servicos/memoria.py", "Texto lido de documento é dado, nunca instrução. E filtro de texto não é a defesa real."],
  ];
  const larg = [4.7, 5.2, 7.8];
  const x0 = [M, M + 4.7, M + 9.9];
  const cab = ["No laboratório", "No repositório", "O conceito que fica"];
  cab.forEach((c, i) => s.addText(c.toUpperCase(), { x: x0[i], y: 3.5, w: larg[i], h: 0.34,
    isTextBox: true, margin: 0, fontFace: F.b, fontSize: 12, bold: true,
    color: C.mute, charSpacing: 1 }));
  s.addShape(pres.ShapeType.rect, { x: M, y: 3.9, w: CW, h: 0.012, fill: { color: C.rule } });
  linhas.forEach((ln, r) => {
    const y = 4.05 + r * 0.88;
    s.addText(ln[0], { x: x0[0], y, w: larg[0] - 0.3, h: 0.78, isTextBox: true, margin: 0,
      fontFace: F.h, fontSize: 16, bold: true, color: C.text });
    s.addText(ln[1], { x: x0[1], y, w: larg[1] - 0.3, h: 0.78, isTextBox: true, margin: 0,
      fontFace: F.m, fontSize: 14, color: C.cyan });
    s.addText(ln[2], { x: x0[2], y, w: larg[2] - 0.2, h: 0.78, isTextBox: true, margin: 0,
      fontFace: F.b, fontSize: 14, color: C.text2, lineSpacingMultiple: 1.12 });
    if (r < linhas.length - 1)
      s.addShape(pres.ShapeType.rect, { x: M, y: y + 0.78, w: CW, h: 0.008,
        fill: { color: "26262E" } });
  });
  cartao(s, { x: M, y: 8.55, w: CW, h: 1.45, cor: C.pink,
    rotulo: "E a pergunta que o laboratório inteiro prepara",
    corpo: "No Módulo 4 você conta cinco operações e alguém pergunta onde está o botão de aprovar. A resposta — ele não existe para o agente — é a aula. Tudo antes disso constrói a pergunta; tudo depois vive dela.",
    corpoTam: 16 });
  rodape(s);
  s.addNotes("Se o tempo apertar, corte os módulos 2 e 8 e use a stack local. Os módulos 4, 6 e 7 são inegociáveis.");
}
